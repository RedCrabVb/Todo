...
export default function NoteScreen({ navigation }: RootTabScreenProps<'Note'>) {
    ...
    return (
        <View>
            ...
            <CustomButton text="Создать заметку" onPress={() => navigation.navigate('Home')} />
            <ScrollView style={{ padding: '5%' }}>
                
                {
                    noteAll && noteAll.filter(t => t.pined == false).map(note => <NoteComponent note={note} key={note.id} items={noteAll} setItems={setNoteAll} loadNote={loadNote}/>)
                }
                {
                    noteAll && noteAll.filter(t => t.pined == true).length != 0 &&
                    <View>
                        <Text>Закрепленные заметки {noteAll.filter(t => t.pined == true).length}</Text>
                        {noteAll.filter(t => t.pined == true).map(note => <NoteComponent note={note} key={note.id} items={noteAll} setItems={setNoteAll} loadNote={loadNote} />)}
                    </View>
                }
            </ScrollView>
        </View>
